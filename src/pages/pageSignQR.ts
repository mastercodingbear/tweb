...
const onFirstMount = async () => {
  ...
  (
    ["Login.QR.Help1", "Login.QR.Help2", "Login.QR.Help3"] as LangPackKey[]
  ).forEach((key) => {
  ...
  const results = await Promise.all([import("qr-code-styling" as any)]);
  ...
  const iterate = async (isLoop: boolean) => {
    try {
      let loginToken = await rootScope.managers.apiManager.invokeApi(
      ...
      if (loginToken._ === "auth.loginTokenMigrateTo") {
        ...
        loginToken = (await rootScope.managers.apiManager.invokeApi(
          "auth.importLoginToken",
          {
            token: loginToken.token,
          },
          options
        )) as AuthLoginToken.authLoginToken;
      }

      if (loginToken._ === "auth.loginTokenSuccess") {
        const authorization =
          loginToken.authorization as any as AuthAuthorization.authAuthorization;
        ...
        if (qrCode._drawingPromise) {
          ...
          promise = Promise.race([
            ...
            new Promise<void>((resolve) => {
              qrCode._canvas._image.addEventListener(
                "load",
                () => {
      ...
      if (isLoop) {
        ...
        await pause(
          diff > FETCH_INTERVAL ? 1e3 * FETCH_INTERVAL : (1e3 * diff) | 0
        );
      }
    } catch (err) {
